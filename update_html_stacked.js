...
let newHtml = html.replace(articleRegex, (match) => {
  ...
  if (originalLogos[name]) {
    return `                <article class="brands__item brands__item--logo brands__item--stacked">
                  <img
                    ...
                    loading="lazy"
                  />
                  <span class="brands__label">${name}</span>
                </article>`;
  }
  
  if (match.includes('brands__item--logo')) {
    let replaced = match.replace('brands__item--logo', 'brands__item--logo brands__item--stacked');
    return replaced.replace('</article>', `  <span class="brands__label">${name}</span>\n                </article>`);
  } else {
    // City Centre Doha is text only
    return match;
  }
});

fs.writeFileSync('brands_html_stacked.txt', newHtml);
